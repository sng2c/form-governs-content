...
- **측정**: `SYNTHESIS CONCLUSION` 마커, 이항 거부, 제3-명제 키워드, **항목 내 novelty(`within_item_novelty`: 같은 모델·같은 항목 3반복 간 shingle 거리 평균 = 반복 분산)**. **핵심 = 인간 $J$**: 종합 문장이 진짜 "양극의 단순 합이 아닌 제3의 명제"인지 정성 판정.

> ⚠️ **novelty 정의 주의**: Exp3의 novelty는 **within-item(반복 간 분산)**이다. Exp2
> `mvp_analysis.json`의 `synth_novelty`(**cross-model: 다른 모델 결론 대비 독창성**)와 정의가
> 달라 직접 비교 불가. 예컨대 본 실험의 `gemma3:1b` 0.995(반복마다 매번 다른 추상 표현)를
> Exp2 `gpt-oss:120b` 0.894(타 모델 대비 독창성)와 같은 축에 놓고 "로컬 1B가 클라우드 120B보다
> 창의적"이라 비교하면 안 된다 — 서로 다른 측정량이다. 본 실험의 novelty는 "형식이 매번 다른
> 종합을 발생시키는가"를 잰 것이지, "타 모델 대비 얼마나 독창적인가"를 잰 것이 아니다.
